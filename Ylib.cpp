...
char *addrtostr(const sockaddr &_addr) {
	...
	auto s = getnameinfo(&_addr, in_len,
						 hbuf, sizeof hbuf,
						 sbuf, sizeof sbuf,
	...
	if (s == 0) {
		...
		//GLOGPRINTF0("getnameinfo()fail, errno=%s\n", strerror(errno));
		address = strdup("");
	}
...
uint32_t crc32(uint32_t crc, const uint8_t *buf, int len) {
	auto endBuff = buf + len;
	crc = ~crc;
	while (buf < endBuff) {
		crc = crc32_table[(crc & 0xFFU) ^ *buf] ^ (crc >> 8U);
		buf++;
	}
	return ~crc;
}
...
class bimap {
	std::unordered_map<A, B *> mapB;
	std::unordered_map<B, A *> mapA;
public:
	int insert(const A &a, const B &b) {
		if (mapA.count(b) or mapB.count(a)) {
			return -1;
		}
	...
	const B &at1(const A &a) const {
		return mapB.at(a);
	}

	const A &at2(const B &b) const {
		return mapA.at(b);
	}

	size_t count1(const A &a) const {
		return mapB.count(a);
	}

	size_t count2(const B &b) const {
		return mapA.count(b);
	}

	void erase1(const A &a) {
		auto search = mapB.find(a);
		if (search != mapB.end()) {
			mapA.erase(*search->second);
			mapB.erase(a);
		}
	}

	void erase2(const B &b) {
		auto search = mapA.find(b);
		if (search != mapA.end()) {
			mapB.erase(*search->second);
			mapA.erase(b);
		}
	}
